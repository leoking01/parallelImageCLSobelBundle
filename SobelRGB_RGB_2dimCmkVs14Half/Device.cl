...
__kernel void SobelFilter3x3Image(
        ...
        __global  __write_only unsigned char *dst
        , int rows
        , int cols
...
//                    youtColor =
//                            ykernelWeights[0] * (float)read_imageui(src, sampler, (int2)(1*(i- 1)+0,j- 1)).w +
//                            ykernelWeights[1] * (float)read_imageui(src, sampler, (int2)(1*(i - 1)+0, j)).w +
//                            ykernelWeights[2] * (float)read_imageui(src, sampler, (int2)(1*(i -1)+0, j+ 1)).w +

//                            ykernelWeights[3] * (float)read_imageui(src, sampler, (int2)(1*(i)+0, j - 1)).w +
//                            ykernelWeights[4] * (float)read_imageui(src, sampler, (int2)(1*(i)+0, j)).w +
//                            ykernelWeights[5] * (float)read_imageui(src, sampler, (int2)(1*(i)+0, j + 1)).w +

//                            ykernelWeights[6] * (float)read_imageui(src, sampler,(int2)(1*(i + 1)+0, j-1)).w +
//                            ykernelWeights[7] * (float)read_imageui(src, sampler, (int2)(1*(i + 1)+0, j)).w +
//                            ykernelWeights[8] * (float)read_imageui(src, sampler,(int2)(1*(i+ 1)+0, j+ 1)).w;

                    // youtColor = (float)read_imageui(src, sampler, (int2)(i, j)  ).w ;
//                     youtColor = (float)read_imageui(src, sampler, (int2)(i, j)  ).w ;
                }
//                  dst[offset] =  255 ;// (unsigned char)(xoutColor + youtColor);
            }
        }
