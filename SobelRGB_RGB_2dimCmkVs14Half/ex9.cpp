...
info()
{
    ...
    printf("全局内存大小： global_mem_size = %lld \n", global_mem_size);
...
cv::Mat imgT;
    cv::cvtColor( src, imgT, cv::COLOR_BGR2BGR555);
    src = imgT;
// cv::cvtColor( imgT, imgT, cv::COLOR_RGBA2BGRA);
//    std::cout<< "src.size() = " <<src.size() <<std::endl;
    ...
    size = src.cols * src.rows * sizeof(unsigned char)*chans ;
    std::cout<< "图形数据规模 size  = " <<size  <<std::endl;

    // cl图像缓存规格
    size_t image_row_pitch = src.cols* chans  ;
    char *buffer = new char[ src.cols * src.rows * src.channels() ];
    memcpy(buffer,  (src.data), src.cols * src.rows * src.channels() );
    void * host_ptr =  NULL;// buffer ;//  NULL;
    cl_int  errcode_ret ;
    ...
    clImageFormat.image_channel_data_type = CL_UNORM_INT_101010;//CL_UNORM_INT8;
    cl_mem memSrc, memDst; //  主机cl缓存
    memSrc = clCreateImage2D(
                context
                 , CL_MEM_READ_WRITE
...
//                CL_ADDRESS_CLAMP,
                CL_FILTER_NEAREST,
                &err);
    std::cout<< "clCreateSampler---err = " <<err <<std::endl;
...
//        std::size_t region[3] = {( std::size_t)src.cols, ( std::size_t)src.rows, ( std::size_t)src.channels() };  //*chans
         std::size_t region[3] = {( std::size_t)src.cols*src.channels(), ( std::size_t)src.rows,  1 };  //*chans
        // 写buffer，表示向gpu内核内存写数据
        err = clEnqueueWriteImage(cmq, memSrc, CL_FALSE, origin, region, src.cols*src.channels(), 0, src.data, 0, NULL, NULL);//*chans
    }
    cl_uint work_dim = 2;
    ...
    global[0] =  src.cols ;//RoundUp(local[0], src.cols) ; //  src.rows;
    global[1] = src.rows;//RoundUp(local[1], src.rows) ;
    ...
    std::cout<< "运行结果 - err  = " <<err  <<std::endl;
     std::cout<< "运行结果 - event  = " <<event  <<std::endl;
      std::cout<< "运行结果 - num_events_in_wait_list  = " <<num_events_in_wait_list  <<std::endl;
    ...
    cv::Mat img2t;
     cv::cvtColor( dst_cl, img2t, cv::COLOR_BGR5552BGR);
    dst_cl= img2t;
  ...
  if( 0 )  cv::Sobel(src, dst_cv, -1, dx,dy,  3, 1, 0, BorderTypes::BORDER_DEFAULT);
  ...
  if( 0 ) imshow("result_OpenCV", dst_cv);
    imshow("result_OpenCL", dst_cl);


    imwrite("result_OpenCL.jpg", dst_cl);
   if(0) imwrite("result_OpenCV.jpg", dst_cv);
