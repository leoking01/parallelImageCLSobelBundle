...
int main()
{
    ...
    Mat src = imread("80modi.jpg",ImreadModes:: IMREAD_GRAYSCALE );
...
// std::size_t region[3] = {( std::size_t)src.rows, ( std::size_t)src.cols, 1};// cl内存中图像数据的区域大小
    ...
    std::cout<< "program_src_len  = " <<program_src_len  <<std::endl;
    ...
    std::cout<< "errcode_ret  = " <<errcode_ret  <<std::endl;
    void *  host_ptr2 = NULL;
    cl_int   errcode_ret2;
    memDst = clCreateBuffer(context, CL_MEM_READ_WRITE, size, host_ptr2,  &errcode_ret2 );
    std::cout<< "errcode_ret2 = " <<errcode_ret2 <<std::endl;
    ...
    cv::Sobel(src, dst_cv, -1, yorder,xorder,  3, 1, 0, BorderTypes::BORDER_DEFAULT);
